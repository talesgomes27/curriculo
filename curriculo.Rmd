---
title: "Tales Wilhame Gomes da Silva <br> Analista de Dados"
output:
  pagedown::html_paged:
    css: ["css/one-column-paged.css", "https://use.fontawesome.com/releases/v5.7.2/css/all.css"]
    number_sections: false
    self_contained: true
links-to-footnotes: false
knit: pagedown::chrome_print

---



```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
library(tidyverse)
library(knitr)
library(lubridate)
#### publications #####
#publications <- read_csv("_content/publications.csv")
#### cv entries #####
cv_entries <- read_csv("_content/cv_entries.csv") %>% 
  mutate(year_begin = year(date)) %>%
  mutate(year_end = case_when(
    date_end == "presente" ~ date_end,
    !is.na(date_end) ~ str_sub(date_end, 1, 4),
    is.na(date_end) ~ date_end
  )) %>%
  mutate(year = ifelse((is.na(year_end) | year_begin == year_end),
                      year_begin,
                      str_c(year_begin, " --- ", year_end)))
```

<div class="grid-container"> 
<!-- top row -->
```{block type="grid-item", echo=TRUE}
## Resumo {.summary}
Sou Engenheiro da computação com conhecimento em Python, R, Sql, Power BI, Tableau, Linux, Excel, Git e outros.
```

```{block type='grid-item', echo=TRUE}
## Contact information {.box}
<div class="info-box">
<script src="https://code.iconify.design/2/2.2.1/iconify.min.js"></script>
| Center | Left |
|:------:|:-----|
| <span class="iconify" data-icon="ic:baseline-place"></span> |Belém, Pará, Brazil|
| <span class="iconify" data-icon="clarity:mobile-phone-solid"></span> | +55 91 98406-9826 |
| <span class="iconify" data-icon="entypo:email"></span> | talesgomes27\@gmail.com |
| <span class="iconify" data-icon="dashicons:admin-site"></span> | [talesgomes.netlify.app/](https://talesgomes.netlify.app/) |
| <span class="iconify" data-icon="simple-icons:datacamp"></span> | [datacamp.com/profile/talesgomes27](https://www.datacamp.com/profile/talesgomes27) |
| <span class="iconify" data-icon="codicon:github"></span> | [github.com/talesgomes27/](https://github.com/talesgomes27/) |
| <span class="iconify" data-icon="akar-icons:linkedin-box-fill"></span> | [linkedin.com/in/talesgomes27/](https://www.linkedin.com/in/talesgomes27/) |
| <span class="iconify" data-icon="bi:person-circle"></span> | [Currículo em inglês](https://talesgomes27.github.io/curriculo/resume.html) |
 
</div>
```

</div>



## <span class="iconify" data-icon="zondicons:education"></span> Educação {.education .nohead .set-width}
```{r}
cv_entries %>% 
  filter(type == 'education') %>% 
  arrange(desc(date_end)) %>%
  mutate(what = str_c(what, ", ", where)) %>% 
  select(year, what) %>% 
  kable()
```




## <span class="iconify" data-icon="eos-icons:big-data"></span> Projetos {.portfolio .nohead .set-width}
```{r}
cv_entries %>% 
  filter(type == 'portfolio') %>%
  mutate(code = ifelse(is.na(code), "", code)) %>%
  mutate(dashboard = ifelse(is.na(dashboard), "", dashboard)) %>%
  mutate(project_link = ifelse(is.na(project_link), "", project_link)) %>%
  mutate(what = str_c(what, ", ", project_link, "  ", dashboard, "  ", code)) %>% 
  select(year, what) %>% 
  kable()
```



## <span class="iconify" data-icon="bxs:certification"></span> Cursos {.certification .nohead .set-width}
```{r}
cv_entries %>% 
  filter(type == 'certification') %>% 
  arrange(desc(date_end)) %>%
  mutate(what = str_c(what, ", ", where)) %>% 
  select(year, what) %>% 
  kable()
```


## <span class="iconify" data-icon="ic:baseline-work"></span> Experiência profissional {.work .nohead .vertical-top .set-width}
```{r}
cv_entries %>% 
  filter(type == 'work', is.na(one_column_paged)) %>% 
  mutate(additional_info = replace_na(additional_info, "")) %>% 
  mutate(what = str_c("<span class='work-title'>", what, "</span>", ", ", "<span class='work-place'>", where, "</span><br class='work-break'>", additional_info)) %>% 
  select(year, what) %>% 
  kable()
```

